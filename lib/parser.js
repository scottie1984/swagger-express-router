...
const validateProperty = (p_Property, p_PropertyName, p_Schema, p_IsParam) => {
	...
	if (p_Schema) {
		...
		if (typeof p_Property !== p_Schema.type && (typeof p_Property !== "number" || p_Schema.type !== "integer") && (p_Schema.type !== "array" || !Array.isArray(p_Property))) {
			...
			switch (p_Schema.type) {
				...
				case "object":
					...
					if (Array.isArray(p_Schema.required)) {
						p_Schema.required.forEach((p_Key) => {
							if (!p_Property.hasOwnProperty(p_Key)) {
								errors.push({ code: errorCodes.REQUEST_ERROR_REQUIRED_ITEM_MISSING, message: `property '${p_PropertyName} is missing required property '${p_Key}'`});
							}
						});
					}

					if (p_Schema.properties) {
						Object.keys(p_Property).forEach((p_Key) => {
							if (p_Schema.properties[p_Key]) {
								validateProperty(p_Property[p_Key], `${p_PropertyName}[${p_Key}]`, p_Schema.properties[p_Key], false).forEach((p_Error) => errors.push(p_Error));
							}
						});
					}
					break;
				case "array":
					...
					p_Property.forEach((p_Item, p_Index) => {
						validateProperty(p_Item, `${p_PropertyName}[${p_Index}]`, p_Schema.items, false).forEach((p_Error) => { errors.push(p_Error); });
					});
					break;
				case "number":
				case "integer":
...
const createParamSchemaValidator = (p_Name, p_Schema) => {
	return {
		name: p_Name,
		schema: p_Schema,
...
const createBodySchemaValidator = (p_Schema) => {
	return (p_Schema) ? {
		schema: p_Schema,
		validate(p_Object) {
			let errors = [];
			...
			if (this.schema.properties) {
				Object.keys(this.schema.properties).forEach((p_Key) => {
					const propSchema = this.schema.properties[p_Key];
					if ((propSchema.required || (this.schema.required && this.schema.required.indexOf(p_Key) !== -1)) && !p_Object.hasOwnProperty(p_Key)) {
						errors.push({ code: errorCodes.REQUEST_ERROR_REQUIRED_ITEM_MISSING, message: `Missing required parameter: '${p_Key}'`});
					}
					if (p_Object.hasOwnProperty(p_Key)) {
...
let resolveRefsForObject = null;
const resolveRefsForArray = (p_Swagger, p_Array) => {
	p_Array.forEach((p_Item) => {
		if (typeof p_Item === "object") {
			if (Array.isArray(p_Item)) {
				resolveRefsForArray(p_Swagger, p_Item);
			} else {
				resolveRefsForObject(p_Swagger, p_Item);
			}
		}
	});
};

resolveRefsForObject = (p_Swagger, p_Object) => {
	Object.keys(p_Object).forEach((p_Key) => {
		const property = p_Object[p_Key];
		if (property && typeof property === "object") {
			if (property.$ref) {
				Object.defineProperty(p_Object, p_Key, { get() { return retrieveRef(property.$ref, p_Swagger);  } });
			} else if (Array.isArray(property)) {
				resolveRefsForArray(p_Swagger, property);
			} else {
				resolveRefsForObject(p_Swagger, property);
			}
		}
	});
};

const resolveRefs = (p_Swagger) => {
	resolveRefsForObject(p_Swagger, p_Swagger);
};

const createValidator = (p_Route, p_Swagger) => {
	const validators = [];
	resolveRefs(p_Swagger);
	...
	if (p_Route.swagger.parameters) {
		p_Route.swagger.parameters.map((p_Parameter) => {
			if (p_Parameter.name && p_Parameter.in) {
				...
				const schema = createParamSchemaValidator(p_Parameter.name, p_Parameter.schema);
	...
	if (p_Route.swagger.requestBody) {
		...
		if (content) {
			const schema = createBodySchemaValidator((content[Object.keys(content)[0] || ""] || {}).schema);
			if (schema) {
