	...
	return {
		...
		validate(p_Request)	{
			...
			if (dataObject || (!dataObject && !this.required)) {
				...
				errors.push({ code: "REQUEST_ERROR_REQUIRED_BODY_MISSING", message: "Missing required body from request" });
			}
			return errors;
		}
	};
...
const validateProperty = (p_Property, p_PropertyName, p_Schema) => {
	const errors = [];

	if (p_Schema) {
		if (typeof p_Property !== p_Schema.type) {
			errors.push(`property '${p_PropertyName}' should be of type '${p_Schema.type}' but was of type '${typeof p_Property}'`);
		} else {
			switch (p_Schema.type) {
				...
				case "object":
					if (p_Schema.maxProperties !== undefined && Object.keys(p_Property).length > Math.max(0, p_Schema.maxProperties)) {
						errors.push(`property '${p_PropertyName}' has '${Object.keys(p_Property).length}' properties which is more than the maximum allowed number of ${p_Schema.maxProperties}'`);
					}
					if (p_Schema.minProperties !== undefined && Object.keys(p_Property).length < Math.max(0, p_Schema.minProperties)) {
						errors.push(`property '${p_PropertyName}' has '${Object.keys(p_Property).length}' properties which is less than the minimum required number of ${p_Schema.minProperties}'`);
					}
					break;
				case "array":
					if (p_Schema.maxItems !== undefined && p_Property.length > Math.max(0, p_Schema.maxItems)) {
						errors.push(`property '${p_PropertyName}' has '${p_Property.length}' items which is more than the maximum allowed number of ${p_Schema.maxItems}'`);
					}
					if (p_Schema.minItems !== undefined && p_Property.length < Math.max(0, p_Schema.minItems)) {
						errors.push(`property '${p_PropertyName}' has '${p_Property.length}' items which is less than the minimal required number of ${p_Schema.minItems}'`);
					}
					if (p_Schema.uniqueItems && !p_Property.every((p_Item, p_Index, p_Array) => p_Array.indexOf(p_Item, p_Index + 1) === -1)) {
						errors.push(`property '${p_PropertyName}' has duplicated items, but is required to have all unique items`);
					}
					break;
				case "number":
					if (p_Schema.multipleOf !== undefined && (p_Schema.multipleOf | 0) > 0 && (p_Property % (p_Schema.multipleOf | 0) !== 0)) {
						errors.push(`property '${p_PropertyName}' with value '${p_Property}' is not a multiple of '${(p_Schema.multipleOf | 0)}'`);
					}
					if (p_Schema.maximum !== undefined && (p_Property > p_Schema.maximum || p_Schema.exclusiveMaximum && p_Property >= p_Schema.maximum)) {
						errors.push(`property '${p_PropertyName}' with value '${p_Property}' is ${(p_Schema.exclusiveMaximum && p_Property === p_Schema.maximum) ? "equal to" : "larger then"} the ${(p_Schema.exclusiveMaximum) ? "exclusive" : ""} maximum of '${p_Schema.maximum}'`);
					}
					if (p_Schema.minimum !== undefined && (p_Property < p_Schema.minimum || p_Schema.exclusiveMinimum && p_Property <= p_Schema.minimum)) {
						errors.push(`property '${p_PropertyName}' with value '${p_Property}' is ${(p_Schema.exclusiveMinimum && p_Property === p_Schema.minimum) ? "equal to" : "smaller then"} the ${(p_Schema.exclusiveMinimum) ? "exclusive" : ""} minimum of '${p_Schema.minimum}'`);
					}
					break;
				case "string":
					if (p_Schema.format) {
						...
						if (!result) {
							errors.push(`property '${p_PropertyName}' did not match the format for '${p_Schema.format}' when testing this value '${p_Property}'`);
						}
					}
					if (p_Schema.pattern) {
						try {
							...
							if (!regexp.test(p_Property)) {
								errors.push(`property '${p_PropertyName}' did not match the pattern '${p_Schema.pattern.toString()}' when testing this value '${p_Property}'`);
							}
						} catch (p_Error) {
					...
					if (p_Schema.minLength !== undefined && p_Property.length < Math.max(0, p_Schema.minLength)) {
						errors.push(`property '${p_PropertyName}' with length ${p_Property.length}, was shorter then the minLength of ${p_Schema.minLength}`);
					}
					if (p_Schema.maxLength !== undefined && p_Property.length > Math.max(0, p_Schema.maxLength)) {
						errors.push(`property '${p_PropertyName}' with length ${p_Property.length}, was longer then the maxLength of ${p_Schema.maxLength}`);
					}
					if (p_Schema.enum) {
						if (Array.isArray(p_Schema.enum) && p_Schema.enum.indexOf(p_Property) === -1) {
							errors.push(`property '${p_PropertyName}' with value ${p_Property}, was not found within the allowed values of '${p_Schema.enum.join("','")}'`);
						} else {
...
const createParamSchemaValidator = (p_Name, p_Schema, p_Swagger) => {
	return {
		...
		validate(p_Param) { return validateProperty(p_Param, this.name, this.schema); },
	};
};
		...
		validate(p_Object) {
			...
			if (this.schema.properties) {
				Object.keys(this.schema.properties).forEach((p_Key) => {
					...
					if (propSchema.required && !p_Object[p_Key]) {
						errors.push({ code: "REQUEST_ERROR_REQUIRED_PARAMETER_MISSING", message: `Missing required parameter: '${p_Key}'`});
					}
					if (p_Object[p_Key]) {
						errors = errors.concat(validateProperty(p_Object[p_Key], p_Key, propSchema));
					}
				});
			} else {
				validateProperty(p_Object, "body", this.schema);
			}
			return errors;
		}
	} : undefined;
	...
	return {
		...
		validate(p_Request) {
			...
			if ((dataObject && dataObject[this.name] !== undefined) || !this.required){
				...
				errors.push({ code: "REQUEST_ERROR_REQUIRED_PARAMETER_MISSING", message: `Missing required parameter: '${this.name}'`});
			}
...
const createValidator = (p_Route, p_Swagger) => {
	const validators = [];
	p_Route.validator = (p_Request) => validators.reduce((p_Previous, p_Validator) => {
		const result = p_Validator.validate(p_Request);
		if (result.length) {
			p_Previous.valid = false;
			p_Previous.errors = p_Previous.errors.concat(result);
		}
		return p_Previous;
	}, { valid: true, errors: [] });
			...
			if (p_Parameter.name && p_Parameter.in) {
				...
				switch (p_Parameter.in) {
					case "path":
						...
						break;
					case "query": validators.push(createValidateObject(true, "query", p_Parameter.name, schema)); break;
					case "header": validators.push(createValidateObject(true, "headers", p_Parameter.name, schema)); break;
					case "cookie": validators.push(createValidateObject(true, "cookies", p_Parameter.name, schema)); break;
